...
<script>
// 返回 key value的对象
import { getkeyValueObjFromDataList, reverseObjectKeyValue } from "@/utils/index";
...
export default {
  ...
  watch: {
    model(val) {
      ...
      if (this.config.default && val.join() === this.config.default.join()) {
        this.fRestModel();
      }
    }
  },
  ...
  methods: {
    Ffilter(arr) {
      let newArr = this.config.listGetter && this.config.listGetter.filter ? this.config.listGetter.filter(arr) : arr;

      this.listKeyMap = getkeyValueObjFromDataList({
        list: newArr,
        keyname:
          this.config.listGetter && this.config.listGetter.optionName ? this.config.listGetter.optionName : "dataname",
        valueName:
          this.config.listGetter && this.config.listGetter.optionValue
