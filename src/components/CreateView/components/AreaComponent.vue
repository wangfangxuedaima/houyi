<template>
  <div class="area-controller flex vertical" :style="{ height: config.height || '400px' }">
    <el-row :gutter="30" class="grow-1 overflow-hidden flex">
      <el-col :span="config.left" class="flex vertical overflow-hidden grow-1">
      ...
      <el-col :span="24 - config.left || 12" class="dash-left grow-1 overflow-hidden flex">
        <div class="right-wrapper grow-1 overflow-auto">
          <div class="tag-container">
            <header class="section-title">已包含</header>
            <el-tag
              v-for="tag in aIncludes"
              :key="tag.title"
              class="eltag"
          ...
          <div class="tag-container">
            <header class="section-title" style="color: #F56C6C;">已排除</header>
            <el-tag
              v-for="tag in aExcludes"
              :key="tag.title"
              class="eltag"
              type="danger"
      ...
      if (this.fCheckRenderInclude(args)) {
        includeBtn = (
          <el-button
            ...
            type="primary"
            class="x-mini-button"
            onClick={() => this.fAddIncludes(node)}>
            {this.includeStr}
          </el-button>
        );
      }
      if (this.fCheckRenderExclude(args)) {
        excludeBtn = (
          <el-button
            ...
            type="danger"
            class="x-mini-button"
            onClick={() => this.fAddExcludes(node)}>
            {this.excludeStr}
          </el-button>
        );
      }
      return (
        <div class="tree-node-container grow-1 flex overflow-hidden">
